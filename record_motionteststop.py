...
def main():
    ...
    while True:
        ...
        #         target_pose = actual.copy()          # 把 target 拉回当前实际位置
        #         arm.arm.rm_movep_canfd(target_pose.tolist(), follow=True)
            
            # if not arm_stopped:
            #     arm.arm.rm_set_arm_stop()
            #     arm.arm.rm_set_arm_delete_trajectory()
            #     arm_stopped = True
            #     print(f"\r  t={now:6.2f}s  input={inp_mag:4.0f}  [STOPPED + queue cleared]     ",
            #           end="", flush=True)
        ...
        ret = arm.arm.rm_movep_canfd(target_pose.tolist(), follow=False)
        if ret != 0:
            consecutive_fails += 1
